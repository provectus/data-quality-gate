        ...
        if reuse_suite:
            if use_old_suite:
                ...
                if not dict_keys:
                    ...
                    old_schema_list = list(
                        suite_old.get_grouped_and_ordered_expectations_by_column()[0].keys())
                    new_schema_list = [
                        x for x in old_schema_list if x not in schema_list]
                    for key in new_schema_list:
                        exp_conf = []
                        exp_conf.append(
                            suite_old.get_grouped_and_ordered_expectations_by_column()[0][key])
                    ...
                    for key, v in list(
                        itertools.zip_longest(
                            schema_list, schema_values)):
                        exp_conf = []
                        if key in suite_old.get_grouped_and_ordered_expectations_by_column()[
                                0]:
                            exp_conf.append(
                                suite_old.get_grouped_and_ordered_expectations_by_column()[0][key])
                            for exps in exp_conf:
                    ...
                    if new_column_in_mapping:
                        ...
                        for name, variable_summary in summary.variables.items():
                            if name in list(new_column_in_mapping.values()):
                                handler.handle(
                                    variable_summary["type"], name, variable_summary, validator)
                    ...
                    new_column_in_mapping_keys = list(
                        filter(r.match, schema_list))
                    schema_list = [
                        x for x in schema_list if x not in new_column_in_mapping_keys]
                    # subtract original suite list keys from nested
                    schema_list = [
                        x for x in schema_list if x not in dict_keys]
                    dict_keys_schema_list = []
                    dict_values_schema_list = []
                    for key in dict_keys:
                        # if nested tables is empty then use just from mapping
                        if not mapping_schema[key]:
                            # create list of lists for nested suites columns
                            dict_keys_schema_list.append(
                                list(mapping_config[key].keys()))
                            dict_values_schema_list.append(
                                list(mapping_config[key].values()))
                        else:  # in other way use key-values from nested mapping
                            # if nested table has renaming
                            dict_keys_schema_list.append(
                                list(mapping_schema[key].keys()))
                            dict_values_schema_list.append(
                                list(mapping_schema[key].values()))
                    dict_suites = []
                    for d_key in dict_keys:
                        suite_old = data_context.get_expectation_suite(
                            f"{d_key}_{old_suite_name}")
                        ...
                        dict_keys_schema_list[list(dict_keys).index(
                            d_key)].append("_nocolumn")
                        ...
                        schema_values = dict_values_schema_list[list(
                            dict_keys).index(d_key)]
                        for key, v in zip(dict_keys_schema_list[list(dict_keys).index(
                                d_key)], schema_values):  # remove table schema test and replace columns name in tests
                            exp_conf = []
                            if key in suite_old.get_grouped_and_ordered_expectations_by_column()[
                                    0]:
                                exp_conf.append(
                    ...
                    for dict_suite in dict_suites:
                        for (key, values) in dict_suite.get_grouped_and_ordered_expectations_by_column()[
                                0].items():
                            for value in values:
                                suite.add_expectation(
                                    ExpectationConfiguration(
                                        kwargs=value["kwargs"],
                                        expectation_type=value["expectation_type"],
                                        meta=value["meta"]))

                    # add expected_table_columns_to_match_set
                    final_schema = sum(
                        dict_values_schema_list, [
                            mapping_schema[x] for x in schema_list])

                    suite.add_expectation(
                        expectation_configuration=ExpectationConfiguration(
                            kwargs={
                                "column_set": final_schema},
            ...
            for name, variable_summary in summary.variables.items():
                ...
                if mapping_schema is not None:
                    if name in list(
                            mapping_schema.keys()) and name not in ignored_columns:
                        handler.handle(
                            variable_summary["type"],
                            name,
                            variable_summary,
                            validator)
                else:
