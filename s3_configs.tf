resource "aws_s3_bucket" "fast_data_qa" {
  bucket_prefix = "${local.resource_name_prefix}-data-qa-gate"
}
...
resource "aws_s3_bucket_object" "great_expectations_yml" {
  bucket       = aws_s3_bucket.fast_data_qa.bucket
...
resource "aws_s3_bucket_object" "test_configs" {
  bucket = aws_s3_bucket.fast_data_qa.bucket
  source = "${path.root}/../configs/test_coverage.json"
  key    = "test_configs/test_coverage.json"
  etag   = filemd5("${path.root}/../configs/test_coverage.json")
}

resource "aws_s3_bucket_object" "expectations_store" {
  for_each = fileset("${path.module}/../expectations_store", "**")
  bucket   = aws_s3_bucket.fast_data_qa.bucket
  source   = "${path.module}/../expectations_store/${each.value}"
  key      = "${aws_s3_bucket.fast_data_qa.bucket}/great_expectations/expectations/${each.value}"
  etag     = filemd5("${path.module}/../expectations_store/${each.value}")
}

## To check with Bogdan
#data "template_file" "test_config_manifest" {
#  template = file("${path.module}/../../../../data-platform/data-qa/configs/manifest.json")
#  vars = {
#    env_name = var.etl_env_name
#  }
#}

#resource "aws_s3_bucket_object" "test_config_manifest" {
#  bucket       = aws_s3_bucket.fast_data_qa.bucket
#  etag         = filemd5("${path.module}/../../../../data-platform/data-qa/configs/manifest.json")
